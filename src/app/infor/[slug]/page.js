    ...
    <div className='page'>
      <div className="film-info">
        <div className="image-film" style={{ background: `url(${dataFilm?.thumb_url})` }}>
          <img className='poster-film' src={dataFilm?.poster_url} alt={`Poster for ${dataFilm?.name}`} />
          <div className="content-film">
            <h2>{dataFilm?.name}</h2>
            <h4>{dataFilm?.origin_name}</h4>
            <div
              className="btn-watch-film"
              onClick={() => handleOpenPopup(episodes[0]?.link_embed)}
            >
              Xem phim
            </div>
          </div>
          ...
          <div className="episodes">
            {episodes.map((item) => (
              <div
                ...
                className="btn-watch"
                onClick={() => handleOpenPopup(item.link_embed)}
              >
                {item.name}
              </div>
            ))}
